...
<body>
  <div class="container" style="max-width: 800px;">
    ...
    <div class="card shadow border-success mx-auto" style="max-width: 500px;">
        <div class="card-header bg-success text-white text-center">Harap dipilih dengan sungguh-sungguh</div>
...
def aksi_login():
    try:
        ...
        # jika ditolak
        return render_template_string(TAMPILAN_HTML, halaman='login', notif="Username/Password Salah!")
    except:
        # jika server login mati, frontend tidak crash, tapi memberi pesan eror
        return render_template_string(TAMPILAN_HTML, halaman='login', notif="Gagal connect ke server login")

@app.route('/logout')
def logout():
    session.pop('admin_user', None) # hapus sesi
    return redirect('/')
...
def menu_admin():
    ...
    list_polling = []
    try:
        # mengambil data riwayat polling dari node result port 5004
...
def aksi_hapus(id_nya):
    if 'admin_user' not in session: return redirect('/')
    try:
        # frontend memberitahu node 3 menghapus id
...
def halaman_token(id_nya):
    if 'admin_user' not in session: return redirect('/')
    try:
        # meminta daftar token yang ada ke node data
...
def halaman_vote(id_nya):
    ...
    try:
        # frontend ambil detail pertanyaan buat ditampilin
...
def aksi_kirim_suara():
    try:
        ...
        data_suara = {
            ...
            'choice': int(request.form['pilihan']) # dijadikan integer/angka
        }
        
        # mengirim suara vote ke node 2 port 5003
...
def halaman_grafik(id_nya):
    try:
        # meminta hasil perhitungan suara dari node data ditampilkan
        r = requests.get(f'{SERVER_DATA}/summary/{id_nya}').json()
